...
impl Ppu {
    ...
    pub fn cycle(&mut self, frame: &mut [u8], memory: &mut Mmu) {
        ...
        match self.mode {
            Mode::HBlank => {
                ...
                if self.dots == 456 {
                    ...
                    memory.set_ly(ly);
                    if ly == 144 {
                        // At line 144, the PPU enters V-Blank
            ...
            Mode::PixelFetcher => {
                // Reading OAM and VRAM takes 168 to 291 dots
                ...
                if self.x == 160 {
                    // When the PPU reaches the end of a scanline, it enters H-Blank
                    self.x = 0;
        ...
        memory.set_stat_mode_flag(match self.mode {
            Mode::HBlank => 0,
            Mode::VBlank => 1,
            Mode::OAM => 2,
            Mode::PixelFetcher => 3,
